**Superseded by** [cfb_sql_course.md](../../learning/sql_course/cfb_sql_course.md)
...
5. Detect any DOUBLE column value that is NaN rather than NULL in `stg.game` (e.g. `excitement`): a value where the number does not equal itself. *Hint: a NaN is the only value for which `x <> x` is TRUE.*
...
4. `list_distinct(provider_key)` per `game_id` from `fact_game_line`. *Hint: `GROUP BY game_id`.*
...
3. Table `MACRO` `sandbox.main.season_games(yr)` returning that season's games. *Hint: `CREATE MACRO ...(yr) AS TABLE SELECT * FROM core.fact_game WHERE season=yr`.*
...
- Build a histogram with `width_bucket`.
...
**The SQL.** DuckDB provides `avg`, `stddev_samp`, `var_samp`, `median`, `quantile_cont`, `mode`, `skewness`, `kurtosis`, and `width_bucket` for binning [duckdb.org/docs/sql/functions/aggregates].
...
### Exercises
1. Histogram of totals with `width_bucket(total, 0, 120, 24)` (5-point bins). *Hint: `GROUP BY` the bucket.*
...
**The SQL.** DuckDB has no built-in p-value function, so approximate the standard normal CDF from the error function: \(\Phi(z) = 0.5\,(1 + \operatorname{erf}(z/\sqrt{2}))\) [3]. DuckDB exposes `erf` [duckdb.org/docs/sql/functions/math] `[uncertain]` — if `erf` is unavailable in your build, use the Abramowitz–Stegun polynomial approximation. The two-proportion z is \(z = (p_1 - p_2)/\sqrt{\hat p(1-\hat p)(1/n_1 + 1/n_2)}\) [4] with pooled \(\hat p\).
...
### Exercises
1. Convert the z-score to a two-sided p-value with the `erf` formula. *Hint: `p = 2*(1 - 0.5*(1+erf(abs(z)/sqrt(2))))`.*
...
**The SQL.** All are arithmetic over `fact_game_line` / `fact_game_odds` columns (`moneyline_home`, `odds`). Bucket predictions with `width_bucket` or `floor`, then compare mean predicted to realised in each bucket.
...
SELECT
  width_bucket(implied_home_prob, 0, 1, 10) AS prob_decile,
  count(*)                                   AS n,
...
### Exercises
1. Conform `stg.games` and `stg.game` into `sandbox.main.game_conformed` with `_provenance IN ('rest','gql','both')` via `UNION BY NAME`. *Hint: select a common column subset from each; add a literal provenance.*
...
- **`coalesce(x, 0)` to "handle" NaN.** NaN passes through. **Fix:** detect with `x <> x`.
