...
No published independent validation found in searched sources for PFF college grades or PFF college signature statistics demonstrating incremental FBS spread, total, CLV, or profitable-after-vig performance over timestamped consensus lines. PFF’s own public validation and methodology discussion is primarily NFL-oriented, so any NFL-to-FBS inference below is explicitly a transfer hypothesis rather than direct college evidence.[^1_2][^1_3][^1_1]

The existing project research likewise found no dedicated academic literature isolating individual FBS tempo or efficiency features against sportsbook totals with documented out-of-sample market-relative results.[^1_4]

## PFF grades

PFF documents that it grades every player on every play on a raw $-2$ to $+2$ scale in 0.5-point increments, where zero represents expected performance. The raw evaluations are transformed into a 0–100 grade intended to summarize performance over a game or season.[^1_3][^1_1]
...
PFF’s public documentation says the 0–100 result derives from play-by-play grading, but it does not disclose a complete formula that would allow independent replication of the football conversion, aggregation weighting, normalization group, or small-sample treatment.[^1_1][^1_3]

PFF-adjacent published descriptions say grades are normalized rather than simply averaged, and that season grades reflect the whole body of work rather than a mean of game grades. That concept is documented more explicitly in PFF’s soccer material than in its football methodology, so do not assume the football implementation is identical.[^1_5]

### Grader workflow

PFF publicly describes trained analysts assigning play-level grades, and public reporting has described a second analyst plus senior adjudication for discrepancies. However, public details are insufficient to calculate inter-rater agreement, percent of plays escalated, error rates by position, or agreement conditional on camera quality.[^1_6][^1_7]
...
ESPN describes FPI as a team-strength measure in points above or below average, with projections derived from that rating. Secondary public summaries describe FPI as EPA-based with Bayesian preseason priors involving prior seasons, returning starters, returning coach, recruiting, and transfers, but use ESPN’s current documentation or archived releases rather than relying on secondary descriptions for production implementation.[^1_8][^1_9]
...
The project research notes that returning-production measures are published preseason and should be archived as-of release because later revisions or re-scraping can produce retrospective leakage.[^1_4]
...
The project’s existing research recommends a market-only calibration regression as a required baseline and identifies market-plus-residual models as the highest-priority modeling family.[^1_10]
...
8. **Use era indicators.** At minimum, separate pre-2023 and post-2023 clock-rule environments.[^1_4][^1_10]
9. **Do not overstate validation.** PFF’s public material documents a charting product and grading system; it does not by itself establish FBS betting value.[^1_3][^1_1][^1_2]
...
[^1_1]: https://www.pff.com/grades

[^1_2]: https://www.pff.com/college

[^1_3]: https://www.pff.com/news/nfl-quarterback-play-level-data

[^1_4]: FBS College Football Pregame Totals Betting System Research Report.md

[^1_5]: https://www.blog.fc.pff.com/blog/pff-fc-grades-explained

[^1_6]: https://247sports.com/college/north-carolina-state/article/pack-football-focus-final-pff-grades-for-nc-state-vs-virginia-in-week-0-289320777/

[^1_7]: https://247sports.com/college/tennessee/longformarticle/how-pro-football-focus-graded-faizon-brandon-college-debut-rest-of-tennessee-football-in-season-opening-win-josh-heupel-289612224/

[^1_8]: https://www.espn.com/college-football/fpi

[^1_9]: https://en.wikipedia.org/wiki/Football_Power_Index

[^1_10]: fbs-totals-frontier-models.md

[^1_11]: https://www.nytimes.com/athletic/6894963/2025/12/17/pff-grades-nfl-games-pro-football-focus/

[^1_12]: https://www.gradientsports.com/how-we-grade

[^1_13]: https://247sports.com/college/usc/article/pff-initial-grades-snap-counts-usc-football-win-fresno-state-289503769/

[^1_14]: https://www.dailynorseman.com/2024/7/20/24198640/analyzing-nfl-analytics-pff-grading

[^1_15]: https://www.covers.com/nfl/key-advanced-metrics-betting-tips

[^1_16]: https://247sports.com/college/iowa/longformarticle/iowa-football-pro-football-focus-grades-analysis-from-win-over-northern-illinois-hank-brown-jeremy-hecklinski-289589570/

[^1_17]: https://thepowerrank.com/guide-cfb-rankings/


---

# How to handle regression to the mean in college football totals

Regression to the mean should be built into your totals model as **partial pooling toward an appropriate prior**, not as an after-the-fact “bounce-back” narrative. In FBS, the correct mean is rarely the national average: it is usually a preseason team prior plus current-year, opponent-adjusted evidence, with heavier shrinkage for volatile components such as turnovers, explosives, red-zone conversion, and special teams.[^2_1][^2_2]
...
- Separate post-2023 observations because the NCAA changed first-down clock behavior and intended to reduce plays per game.[^2_5][^2_6]
...
The 2023 NCAA rules changed clock operation after first downs outside the final two minutes of each half, making pre- and post-rule-change pace distributions non-interchangeable without an era effect or separate model.[^2_6][^2_5]
...
The project research identifies distributional models such as NGBoost or GAMLSS as useful experiments because they can estimate conditional mean and variance jointly; they should be evaluated via proper scoring rules and held-out interval coverage, not just average-error reduction.[^2_7]
...
- **Do not let the current game enter the aggregate.** Every rolling metric must end at the prior completed game.[^2_1]
- **Do not tune shrinkage constants on the test season.** Fit them inside each expanding-window training period.
- **Do not claim betting value from a better score forecast alone.** Compare against the decision-time market, then separately test CLV and vig-adjusted profit.[^2_7]
...
Use identical seasons, games, line timestamps, vig assumptions, and retraining cadence. Report RMSE/MAE for total residual, CRPS if probabilistic, calibration by edge bucket, CLV, and realized returns separately. The project research recommends precisely this market-plus-feature residual setup and insists that all comparisons share the same game universe, line source, timestamps, and chronological test window.[^2_7]
...
[^2_1]: FBS College Football Pregame Totals Betting System Research Report.md

[^2_2]: https://pmc.ncbi.nlm.nih.gov/articles/PMC5969004/

[^2_3]: https://thepowerrank.com/2018/08/14/how-to-make-accurate-football-predictions-with-linear-regression/

[^2_4]: https://www.espn.com/college-football/story/\_/id/48288665/college-football-2026-turnovers-lucky-bounces

[^2_5]: https://footballfoundation.org/news/2023/7/20/important-rule-changes-for-the-2023-college-football-season.aspx

[^2_6]: https://www.espn.com/college-football/story/\_/id/36255797/ncaa-approves-rule-change-run-clock-first-downs

[^2_7]: fbs-totals-frontier-models.md

[^2_8]: https://harvardsportsanalysis.org/2014/10/how-random-are-turnovers/

[^2_9]: https://www.puntandrally.com/backtest_regression.php

[^2_10]: https://www.nytimes.com/athletic/4805119/2023/08/25/college-football-clock-rule-changes/

[^2_11]: https://pistolsfiringblog.com/stat-of-the-week-new-clock-rules-meaning-less-plays-a-game-for-oklahoma-state/

[^2_12]: https://www.cbssports.com/college-football/news/despite-running-fewer-plays-college-football-games-are-actually-getting-longer-so-whos-to-blame/

[^2_13]: https://ncaaorg.s3.amazonaws.com/championships/sports/football/rules/2026PRMFB_2026RuleChanges.pdf

[^2_14]: https://link.springer.com/chapter/10.1007/978-3-031-89175-5_27

[^2_15]: https://nflanalytic.com/explainer-turnovers-luck.html

[^2_16]: https://www.mdpi.com/2075-4663/5/1/1

[^2_17]: https://drbobsports.com/2016-nfl-turnover-luck/


---

# How to calculate team-specific preseason priors for CFB

A strong CFB preseason prior is a **team-specific forecast of latent offense, defense, pace, and special-teams quality before Week 1**, built from prior performance but rebuilt for the current roster, staff, and era. For a totals model, avoid one “team power” prior; produce separate priors for offensive efficiency, defensive efficiency, neutral pace, explosiveness, turnover propensity, and special teams.[^3_1][^3_2]
...
Use these as priors for Week 1, then blend them with strictly entering-game evidence as the season progresses. CFBD supports game, team, player, recruiting, rating, and analytics data, including team PPA/EPA-style metrics, returning production, recruiting, transfer portal, and SP+ endpoints.[^3_3][^3_4][^3_5][^3_6]
...
For offensive continuity, ESPN/Bill Connelly’s published 2026 returning-production construction weights returning offensive-line snaps at 39.6%, receiver/tight-end receiving yards at 35.0%, QB passing yards at 22.3%, and running-back rushing yards at 3.1%. That framework is useful as a benchmark, but you should replace its generic production weights with weights fitted separately for your targets: offensive EPA, neutral pace, and points per possession.[^3_7]

For defense, ESPN’s 2026 construction uses returning snaps, tackles, and tackles for loss, with listed weights of 65.9%, 19.2%, and 14.9%, respectively. For totals, augment this with returning coverage snaps, pass-rush pressure share, and defensive-back/LB participation if your PFF feed can be historically timestamped.[^3_7]
...
PFF public documentation confirms the broad $-2$ to $+2$ raw grading scale and 0–100 transformed grades, but its football aggregation and normalization formulas are proprietary. Treat the delivered grades as vendor features to recalibrate—not as direct outcome-scale estimates.[^3_8][^3_9]
...
Your required standard is incremental value over a decision-time total. The project research found no direct FBS totals evidence that returning production, recruiting, pace, EPA, or matchup features independently beat the market, so use those variables as testable priors rather than assumed edges.[^3_2][^3_10]
...
[^3_1]: https://www.espn.com/college-football/story/\_/id/49593338/final-preseason-college-football-sp+-rankings-takeaways-2026

[^3_2]: FBS College Football Pregame Totals Betting System Research Report.md

[^3_3]: https://api.collegefootballdata.com/getting-started

[^3_4]: https://api.collegefootballdata.com/api/ratings

[^3_5]: https://cfbfastr.sportsdataverse.org/reference/index.html

[^3_6]: https://github.com/CFBD/cfb.js?ref=blog.collegefootballdata.com

[^3_7]: https://www.espn.com/college-football/story/\_/id/48259759/college-football-returning-production-2026-notre-dame-texas

[^3_8]: https://www.pff.com/grades

[^3_9]: https://www.pff.com/news/nfl-quarterback-play-level-data

[^3_10]: fbs-totals-frontier-models.md

[^3_11]: https://www.espn.com/college-football/fpi

[^3_12]: https://www.cbssports.com/college-football/news/college-football-returning-production-2026-returning-starters-fbs/

[^3_13]: https://stadiumsportzz.com/2023/10/08/unlocking-espns-football-power-index-a-comprehensive-overview-with-key-takeaways/

[^3_14]: https://www.splitzoneduo.com/p/cfb-returning-production-2026

[^3_15]: https://www.secrant.com/rant/sec-football/espn---2025-college-football-bill-connelly-returning-production-improved-regressing-teams/117664974/

[^3_16]: https://docs.composio.dev/toolkits/college_football_data

[^3_17]: https://www.espn.com/college-football/fpi/\_/group/1Somehow

[^3_18]: https://www.espn.com/college-football/story/\_/id/49317269/2026-college-football-top-25-x-factors-team

[^3_19]: https://x.com/ESPN_BillC/status/1528723671950295042
...
CFBD exposes game, player, recruiting, rankings, analytics, ratings, and returning-production-related data, making it a workable backbone for the baseline version.[^4_2][^4_3][^4_4]
...
ESPN has publicly described a related four-component preseason FPI framework: prior performance, returning starters, recruiting rankings, and coaching tenure. It also states that prior information remains useful even after in-season evidence accumulates, rather than disappearing completely.[^4_1]
...
ESPN’s returning-production methodology explicitly treats continuity as position-weighted rather than a simple count of returning starters; its 2026 offensive version places substantial weight on returning offensive-line snaps, receiving production, and QB production.[^4_5]
...
The project’s existing research identifies this market-plus-feature residual approach as the priority candidate, while noting that direct FBS evidence that individual tempo, EPA, roster, or matchup metrics beat totals markets is absent in the searched sources.[^4_6][^4_7]
...
Walk-forward testing and strict timestamp alignment are fundamental requirements for credible backtesting because future information must not enter an earlier decision.[^4_8][^4_9]
...
The project research specifically recommends comparing raw market total, a market-only regression, a regularized model, an opponent-adjusted rating model, and a market-residual model under the identical game universe and timestamps.[^4_7]
...
The 2023 NCAA rule changes altered clock operation after first downs and were intended to reduce plays; include a regime flag or train era-specific pace components rather than assuming 2012 pace translates directly to 2025 pace.[^4_10][^4_11]
...
<span style="display:none">[^4_12][^4_13][^4_14][^4_15][^4_16][^4_17][^4_18][^4_19][^4_20][^4_21][^4_22][^4_23]</span>

<div align="center">⁂</div>

[^4_1]: https://www.espn.com/blog/statsinfo/post/\_/id/109828/reintroducing-espns-college-football-power-index

[^4_2]: https://api.collegefootballdata.com/getting-started

[^4_3]: https://api.collegefootballdata.com/api/ratings

[^4_4]: https://cfbfastr.sportsdataverse.org/reference/index.html

[^4_5]: https://www.espn.com/college-football/story/\_/id/48259759/college-football-returning-production-2026-notre-dame-texas

[^4_6]: FBS College Football Pregame Totals Betting System Research Report.md

[^4_7]: fbs-totals-frontier-models.md

[^4_8]: https://portfoliooptimizationbook.com/book/8.4-backtesting-market-data.html

[^4_9]: https://mbrenndoerfer.com/writing/backtesting-trading-strategies-simulation-frameworks

[^4_10]: https://footballfoundation.org/news/2023/7/20/important-rule-changes-for-the-2023-college-football-season.aspx

[^4_11]: https://www.espn.com/college-football/story/\_/id/36255797/ncaa-approves-rule-change-run-clock-first-downs

[^4_12]: https://www.reddit.com/r/CFB/comments/1imi771/correlation_between_returning_production_and/

[^4_13]: https://arxiv.org/html/2506.03057v1

[^4_14]: https://libres.uncg.edu/ir/asu/f/Singleton_Sydney_2019_Thesis.pdf

[^4_15]: https://www.sciencedirect.com/science/article/abs/pii/S0169207011000914

[^4_16]: https://appstate.figshare.com/articles/thesis/Using_Recruiting_Rankings_And_Returning_Team_Measurements_To_Predict_College_Football_Team_Success/29863847

[^4_17]: https://thepowerrank.com/2018/08/14/how-to-make-accurate-football-predictions-with-linear-regression/

[^4_18]: https://www.atiner.gr/presentations/Theodore-Trafalis.pdf

[^4_19]: https://www.theonlycolors.com/2020/7/29/21346008/hopelessly-optimistic-college-football-preview-part-1-methodology

[^4_20]: https://sportsdata.io/developers/workflow-guide/ncaa-football

[^4_21]: https://www.backtestbase.com/education/how-many-trades-for-backtest

[^4_22]: https://www.stonex.com/en/business/financial-glossary/backtesting/

[^4_23]: https://www.reddit.com/r/algotrading/comments/1et9k3v/where_do_you_get_your_data_for_backtesting_from/
...
| :-- | :-- | :-- | :-- |
| Neutral pace / seconds per play | Team preseason pace, then conference-era mean | Moderate | Pace has some coaching persistence but also strong game-state and opponent effects.[^5_2][^5_5] |
| Offensive EPA/play | Team preseason offense and opponent-adjusted tier mean | Moderate | It contains real signal, but schedule and small samples distort early values.[^5_2] |
| Defensive EPA/play allowed | Team preseason defense and opponent-adjusted tier mean | Moderate-high | Defensive samples are noisier and more opponent-dependent.[^5_2] |
| Explosive-play rate | Team prior and FBS/conference mean | High | Rare events dominate short samples.[^5_2] |
| Turnover margin | Zero or expected turnover differential | Very high | Turnover performance regresses hard because realization has a large luck component.[^5_4][^5_6] |
| Fumble recovery rate | 50% baseline | Very high | Recovery is highly bounce-driven.[^5_4][^5_7] |
| Sack rate | Pressure rate plus conversion prior | Moderate | Sacks combine repeatable pressure with noisy conversion and QB style.[^5_2] |
| Special teams | Specialist prior plus tier mean | High early | Attempt volume is low and weather-sensitive.[^5_2] |

### Turnovers

Turnovers are the clearest case where raw outcomes should be decomposed into opportunity and realization. An NFL study found that teams with strong season-to-date turnover performance tend to regress toward the mean, which is useful as mechanism evidence but remains NFL evidence rather than direct FBS validation.[^5_4]
...
Raw plays per game is not a clean pace statistic because it mixes both teams' behavior, game script, turnovers, and overtime. Pace features should instead be based on neutral-state offensive and defensive pace calculated only through prior completed games.[^5_2]

That matters even more because 2023 NCAA timing changes altered the clock after first downs outside the final two minutes and were expected to reduce plays per game and game length. Official and media descriptions of the 2023 changes confirm the direction of the mechanical effect, while the project research summarizes early analysis estimating roughly 7.8% fewer plays and about 1.4% shorter games post-change.[^5_5][^5_8][^5_2]
...
Prior-year team performance is not the same thing as current-team quality, especially in the portal era. The prior should be rebuilt using returning production, projected snap continuity, transfer additions and losses, quarterback continuity, and coordinator continuity.[^5_11][^5_2]
...
If a licensed PFF feed is historically point-in-time valid, the stronger version of a preseason prior comes from player-level reconstruction rather than last season's final team grade. PFF publicly documents its raw $-2$ to $+2$ play grading scale and 0-100 transformed grades, but not the full football aggregation formula, so these fields should be treated as vendor features to recalibrate rather than direct outcome estimates.[^5_12][^5_13]
...
The right evaluation target is not simply the game total points scored. The more useful target for betting research is the amount of information left unexplained by the decision-time market total.[^5_3]
...
The project research explicitly recommends a market-only baseline, a calibrated market-only regression, a regularized linear model, an opponent-adjusted rating model, and a market-residual model, all compared on the same game universe, timestamps, and line source.[^5_3]
...
- Clock-rule era flag.
- Neutral-site flag.
- Uncertainty measures for roster and staff turnover.[^5_2][^5_3]
...
Use expanding-window chronological testing. For example, 2017 should be trained only on 2012-2016, 2018 only on 2012-2017, and so on through the available history.[^5_3]
...
5. Pool only genuinely out-of-sample predictions when reporting performance.[^5_14][^5_15][^5_3]

The project research emphasizes that all comparisons must share the same game universe, timestamps, line source, and chronological test window, and that line-field fidelity itself should be audited before downstream claims are trusted.[^5_3]
...
The project research notes that nearly every model family under consideration has no direct published FBS totals evidence at the incremental-beyond-market, CLV, or profitable-after-vig levels, so residual accuracy and calibration should be treated as necessary but not sufficient standards.[^5_3]
...
2. Build prior-year opponent-adjusted offense, defense, and neutral-pace summaries from CFBD play-by-play or advanced team metrics.[^5_16][^5_1]
3. Fit a simple ridge-based preseason prior model for offense, defense, and pace.
4. Generate market-residual targets from audited decision-time totals.[^5_3]
5. Compare market-only, market-plus-raw-prior, and market-plus-roster-adjusted-prior models in expanding windows.[^5_3]
6. Add PFF player-level reconstruction only after proving point-in-time coverage and showing early-season lift over simpler baselines.[^5_13][^5_12]

## Practical takeaway

The conversation's main modeling recommendation is to make preseason priors the anchor for early-season college football totals, then phase them out gradually as opponent-adjusted current-season evidence accumulates. Those priors should be roster-aware, staff-aware, era-aware, and tested only inside a strict market-residual backtest with identical timestamps, line definitions, and chronological validation windows.[^5_10][^5_2][^5_3]

<div align="center">⁂</div>

[^5_1]: https://api.collegefootballdata.com/getting-started

[^5_2]: FBS College Football Pregame Totals Betting System Research Report.md

[^5_3]: fbs-totals-frontier-models.md

[^5_4]: https://pmc.ncbi.nlm.nih.gov/articles/PMC5969004/

[^5_5]: https://footballfoundation.org/news/2023/7/20/important-rule-changes-for-the-2023-college-football-season.aspx

[^5_6]: https://thepowerrank.com/2018/08/14/how-to-make-accurate-football-predictions-with-linear-regression/

[^5_7]: https://www.espn.com/college-football/story/\_/id/48288665/college-football-2026-turnovers-lucky-bounces

[^5_8]: https://www.espn.com/college-football/story/\_/id/36255797/ncaa-approves-rule-change-run-clock-first-downs

[^5_9]: https://www.espn.com/college-football/story/\_/id/49593338/final-preseason-college-football-sp+-rankings-takeaways-2026

[^5_10]: https://www.espn.com/blog/statsinfo/post/\_/id/109828/reintroducing-espns-college-football-power-index

[^5_11]: https://www.espn.com/college-football/story/\_/id/48259759/college-football-returning-production-2026-notre-dame-texas

[^5_12]: https://www.pff.com/grades

[^5_13]: https://www.pff.com/news/nfl-quarterback-play-level-data

[^5_14]: https://portfoliooptimizationbook.com/book/8.4-backtesting-market-data.html

[^5_15]: https://mbrenndoerfer.com/writing/backtesting-trading-strategies-simulation-frameworks

[^5_16]: https://api.collegefootballdata.com/api/ratings


---

# how to structure market residual regression models

Structure the market-residual model as a **hierarchy of increasingly difficult questions**: first calibrate the market, then test whether a compact set of pregame features explains what remains unexplained. Your primary target should be the final-total residual at an auditable decision-time quote—not the raw final total.[^6_1][^6_2]
...
Do not train on closing lines if your production decision happens at opening or Wednesday prices. That would create direct lookahead leakage. The project research specifically treats the residual of final total minus decision-time market total as the appropriate incremental-information target.[^6_2][^6_1]
...
The project research recommends market-only, calibrated market-only, regularized, opponent-adjusted, and residual-model baselines, all evaluated using the same market source, universe, timestamp, and chronological split.[^6_2]
...
The decision-time total, exact sportsbook, timestamp, and odds are foundational. Historical FBS odds with confirmed sportsbook/time metadata are limited in some sources, so stored line fields should be audited before model claims are trusted.[^6_1][^6_2]
...
All values must be computed only through each team’s prior completed game. The project research recommends opponent-adjusted trailing pace and efficiency estimates, neutral/garbage-time-aware construction, and shrinkage of early-season data toward relevant means.[^6_1]
...
PFF fields must have demonstrable historical availability. If grades are delivered only as final-season or post-game revised snapshots, they cannot be used as historical weekly pregame inputs. The project research identifies revision, coverage heterogeneity, and endpoint timing as key risks for any premium-stat feed.[^6_1][^6_2]
...
The project research identifies NGBoost/GAMLSS as a sensible later experiment for predicting both the conditional mean and variance of total residuals, with CRPS and held-out interval coverage as the appropriate tests.[^6_2]
...
The project research emphasizes that the 2023 rule change is a structural pace regime break and that nested chronological evaluation applies without exception.[^6_2][^6_1]
