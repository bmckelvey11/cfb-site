 ...
 * useConnection, useConnectionStatus, useExport, and MotherDuckSDKProvider.
 */
    ...
    if (isLoading && options?.placeholderData !== undefined) {
      const ph =
        typeof options.placeholderData === 'function'
          ? (options.placeholderData as (p: TData | undefined) => TData | undefined)(
              transformed,
            )
...
// Local stand-in for the production persistent dive state.
const diveStateStore = new Map<string, unknown>();
const diveStateListeners = new Map<string, Set<() => void>>();

export function useDiveState<T>(key: string, initial: T): [T, (v: T) => void] {
  const [, force] = useState(0);
  useEffect(() => {
    const listeners = diveStateListeners.get(key) ?? new Set<() => void>();
    const cb = () => force((n) => n + 1);
    listeners.add(cb);
    diveStateListeners.set(key, listeners);
    return () => {
      listeners.delete(cb);
    };
  }, [key]);
  const value = (diveStateStore.has(key) ? diveStateStore.get(key) : initial) as T;
  const setValue = useCallback(
    (next: T) => {
      if (next === undefined) diveStateStore.delete(key);
      else diveStateStore.set(key, next);
      diveStateListeners.get(key)?.forEach((l) => l());
    },
    [key],
  );
