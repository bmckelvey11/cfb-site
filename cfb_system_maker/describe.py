...
def describe(system: SystemFilter) -> list[dict[str, object]]:
    ...
    for filt in system.feature_filters:
        ...
        if feature is None:
            sentences.append(
                {"text": f'Unknown filter "{filt.key}" is unavailable', "key": f"ff:{filt.key}"}
            )
            continue

        if feature.team_scoped and filt.perspective in _PERSPECTIVE_PREFIX:
            label = f"{_PERSPECTIVE_PREFIX[filt.perspective]} {feature.label}"
        else:
            label = feature.label
