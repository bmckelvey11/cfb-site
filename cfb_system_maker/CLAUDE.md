...
`actionnetwork` is a fourth acquire path (no CFBD token): per-book odds including 1H/1Q markets CFBD lacks → `data/raw/actionnetwork/`. Browser-ish UA required. **Retired 2026-09-11** — not scheduled, do not re-run; the files on disk stay loaded and `stg.an_*` keeps building from them. Only `CFB-AN-History` (`research/spread/scripts/collect_line_timing.py`) still hits Action Network, through the 2026 season. Why: `docs/odds-sources-an-vs-apis-2026-09-11.md`.
...
- **Action Network movement:** every table above is a **snapshot** — one closing price per
  ...
  `scripts/check_an_tick_pin.py` verifies it after a rebuild and tells a schema drift
  apart from a stale CSV.
