...
This file is project conventions and commands for agents. Behavioral defaults (think / simplicity / surgical / verify) live in `~/.claude/CLAUDE.md`.
...
In-tree siblings (not the Flask app): `over_zero/` is merged Arscott/floor-bias research — run its commands from that cwd. `cfb_totals_model` is the GBM totals harness (`python -m cfb_totals_model backtest --line ou_open --permute`). Cite opening totals `ou_open`, 2022–25 prior-season folds, `|edge| >= 0`; week 1 / fewer than three prior games is no-bet under default `--min-prior-games 3`. Do not cite leaked-era 57% / +8.82% ROI.
...
`docs/superpowers/specs/2026-06-12-cfb-system-maker-design.md` describes original v1 intent (CLI-first). Bet Labs UI + later pipeline live in code; prefer this file and `.planning/PROJECT.md` over `docs/PROJECT_MAP.md` (stale as of 2026-07-16).
