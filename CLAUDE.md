...
`scrapers.py` is a parallel, broader fetch path: a registry (`ENDPOINTS`, 73 entries) covering every CFBD REST endpoint **the vendored client exposes**, driven by one runner (`scrape`). The live CFBD spec has 74 paths; 73 are registered and `/info/usage` is account metering, deliberately unregistered. (The vendored client was bumped `034cd17` -> `52f2bbf` on 2026-08-28 to expose the last 10 — CFP, core ratings, expanded SRS, coach profile/seasons/tenures, conference affiliations/changes.) Re-check coverage any time with `python scripts/audit_coverage.py` (registry vs disk) and `python scripts/audit_endpoints.py` (live spec vs registry — partitions all 74 spec paths); `docs/data-coverage.md` records *why* each absent endpoint is absent (deliberate, deferred on cost) and why the empty `[]` files are data floors rather than failures. It writes raw JSON to `data/raw/` only — it does not feed `build`/`backtest` (which still use `fetch`'s `games`/`lines` files). Intended as the load source for a future Postgres `JSONB` staging layer.

`graphql_client.py` is a **third** fetch path over the CFBD GraphQL (Hasura) API (`graphql_scrape`, CLI `graphql`) — **Patreon Tier 3 only**. It introspects the schema once, then for each root table in `GQL_DEFAULT_TABLES` (35 tables; `GQL_EXCLUDED` holds the two deliberate omissions with reasons, and `scripts/audit_endpoints.py` partitions the 37 introspected tables against both) auto-selects scalar columns (skips relations), picks a sort key (`id` else first scalar), and paginates with `limit`/`offset` — collapsing per-row REST fan-out into a few bulk queries. Writes to `data/graphql/{table}.json` (separate from `data/raw/` because GraphQL row shapes differ from REST). Key details: root fields advertise their own args via introspection, so tables lacking `limit`/`offset` are fetched in one shot, and `where:{season:{_in:[…]}}` is only added when the table has a season/year column **and** a `where` arg. Hasura names the sort arg **`orderBy`** and takes an **uppercase** enum (`orderBy: {id: ASC}`) — `order_by`/`asc` are both rejected, and unsorted `limit`/`offset` has no stable row order, so pages can skip or repeat rows. The HTTP poster must send a non-default `User-Agent` (Cloudflare 403s urllib's default); `post_fn` is injectable for network-free tests.
