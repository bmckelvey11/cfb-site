...
The existing project research likewise found no dedicated academic literature isolating individual FBS tempo or efficiency features against sportsbook totals with documented out-of-sample market-relative results. *[FBS College Football Pregame Totals Betting System Research Report.md]*
...
The project research notes that returning-production measures are published preseason and should be archived as-of release because later revisions or re-scraping can produce retrospective leakage. *[FBS College Football Pregame Totals Betting System Research Report.md]*
...
The project’s existing research recommends a market-only calibration regression as a required baseline and identifies market-plus-residual models as the highest-priority modeling family. *[fbs-totals-frontier-models.md]*
...
8. **Use era indicators.** At minimum, separate pre-2023 and post-2023 clock-rule environments. *[FBS College Football Pregame Totals Betting System Research Report.md]*
...
- **Do not let the current game enter the aggregate.** Every rolling metric must end at the prior completed game. *[FBS College Football Pregame Totals Betting System Research Report.md]*
- **Do not tune shrinkage constants on the test season.** Fit them inside each expanding-window training period.
- **Do not claim betting value from a better score forecast alone.** Compare against the decision-time market, then separately test CLV and vig-adjusted profit. *[fbs-totals-frontier-models.md]*
...
Use identical seasons, games, line timestamps, vig assumptions, and retraining cadence. Report RMSE/MAE for total residual, CRPS if probabilistic, calibration by edge bucket, CLV, and realized returns separately. The project research recommends precisely this market-plus-feature residual setup and insists that all comparisons share the same game universe, line source, timestamps, and chronological test window. *[fbs-totals-frontier-models.md]*
...
Your required standard is incremental value over a decision-time total. The project research found no direct FBS totals evidence that returning production, recruiting, pace, EPA, or matchup features independently beat the market, so use those variables as testable priors rather than assumed edges. *[FBS College Football Pregame Totals Betting System Research Report.md]*
...
The project research specifically recommends comparing raw market total, a market-only regression, a regularized model, an opponent-adjusted rating model, and a market-residual model under the identical game universe and timestamps. *[fbs-totals-frontier-models.md]*
...
The core premise is that most public or vendor football metrics are useful first as descriptive summaries and only secondarily as betting features. The existing project research found no direct FBS totals-market evidence showing that isolated tempo, EPA, roster, or matchup features beat the market on their own, so each feature should be treated as a testable input rather than an assumed edge. *[FBS College Football Pregame Totals Betting System Research Report.md]*
...
- Test those priors only in a decision-time market-residual framework rather than against raw game totals alone. *[fbs-totals-frontier-models.md]*
...
| --- | --- | --- | --- |
| Neutral pace / seconds per play | Team preseason pace, then conference-era mean | Moderate | Pace has some coaching persistence but also strong game-state and opponent effects. *[FBS College Football Pregame Totals Betting System Research Report.md]* |
| Offensive EPA/play | Team preseason offense and opponent-adjusted tier mean | Moderate | It contains real signal, but schedule and small samples distort early values. *[FBS College Football Pregame Totals Betting System Research Report.md]* |
| Defensive EPA/play allowed | Team preseason defense and opponent-adjusted tier mean | Moderate-high | Defensive samples are noisier and more opponent-dependent. *[FBS College Football Pregame Totals Betting System Research Report.md]* |
| Explosive-play rate | Team prior and FBS/conference mean | High | Rare events dominate short samples. *[FBS College Football Pregame Totals Betting System Research Report.md]* |
...
| Sack rate | Pressure rate plus conversion prior | Moderate | Sacks combine repeatable pressure with noisy conversion and QB style. *[FBS College Football Pregame Totals Betting System Research Report.md]* |
| Special teams | Specialist prior plus tier mean | High early | Attempt volume is low and weather-sensitive. *[FBS College Football Pregame Totals Betting System Research Report.md]* |
...
Raw plays per game is not a clean pace statistic because it mixes both teams' behavior, game script, turnovers, and overtime. Pace features should instead be based on neutral-state offensive and defensive pace calculated only through prior completed games. *[FBS College Football Pregame Totals Betting System Research Report.md]*
...
The right evaluation target is not simply the game total points scored. The more useful target for betting research is the amount of information left unexplained by the decision-time market total. *[fbs-totals-frontier-models.md]*
...
The project research explicitly recommends a market-only baseline, a calibrated market-only regression, a regularized linear model, an opponent-adjusted rating model, and a market-residual model, all compared on the same game universe, timestamps, and line source. *[fbs-totals-frontier-models.md]*
...
- Clock-rule era flag.
- Neutral-site flag.
- Uncertainty measures for roster and staff turnover. *[FBS College Football Pregame Totals Betting System Research Report.md]*
...
Use expanding-window chronological testing. For example, 2017 should be trained only on 2012-2016, 2018 only on 2012-2017, and so on through the available history. *[fbs-totals-frontier-models.md]*
...
The project research emphasizes that all comparisons must share the same game universe, timestamps, line source, and chronological test window, and that line-field fidelity itself should be audited before downstream claims are trusted. *[fbs-totals-frontier-models.md]*
...
4. Generate market-residual targets from audited decision-time totals. *[fbs-totals-frontier-models.md]*
5. Compare market-only, market-plus-raw-prior, and market-plus-roster-adjusted-prior models in expanding windows. *[fbs-totals-frontier-models.md]*
...
Structure the market-residual model as a **hierarchy of increasingly difficult questions**: first calibrate the market, then test whether a compact set of pregame features explains what remains unexplained. Your primary target should be the final-total residual at an auditable decision-time quote—not the raw final total. *[FBS College Football Pregame Totals Betting System Research Report.md]*

---

# Market-Residual Totals Model: Targets, Features, and Validation
...
Do not train on closing lines if your production decision happens at opening or Wednesday prices. That would create direct lookahead leakage. The project research specifically treats the residual of final total minus decision-time market total as the appropriate incremental-information target. *[fbs-totals-frontier-models.md]*
...
The project research recommends market-only, calibrated market-only, regularized, opponent-adjusted, and residual-model baselines, all evaluated using the same market source, universe, timestamp, and chronological split. *[fbs-totals-frontier-models.md]*
...
PFF fields must have demonstrable historical availability. If grades are delivered only as final-season or post-game revised snapshots, they cannot be used as historical weekly pregame inputs. The project research identifies revision, coverage heterogeneity, and endpoint timing as key risks for any premium-stat feed. *[FBS College Football Pregame Totals Betting System Research Report.md]*
...
The project research identifies NGBoost/GAMLSS as a sensible later experiment for predicting both the conditional mean and variance of total residuals, with CRPS and held-out interval coverage as the appropriate tests. *[fbs-totals-frontier-models.md]*
...
The project research emphasizes that the 2023 rule change is a structural pace regime break and that nested chronological evaluation applies without exception. *[fbs-totals-frontier-models.md]*
