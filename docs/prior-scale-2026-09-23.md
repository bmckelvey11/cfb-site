...
on a grid edge. The 0.24-point gain over $k=0$ is a tuning-set difference on the same games
that chose it, with no interval; it says the fitted carryover is not too strong on pre-2021
data, not that priors are confirmed.
...
- **One look counts:** the run after the last 2026 regular-season week is complete.
  **Confirmed** if early 2+ *improves* and primary is not *worse*; *matches* is recorded as
  unconfirmed. About 14 week-clusters, so the interval will be wide.
...
- The robustness failure on 2021–2025 (carryover ×1.5) is consistent with this curve, which
  also rises past $k=1$; nothing here argues for shrinking the carryover below its fitted
  value.
...
- **Not a reason to read ×0.5 off 2021–2025.** Pre-2021 data prefers the full carryover.
