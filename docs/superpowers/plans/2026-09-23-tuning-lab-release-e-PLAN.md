...
| 6. Period verdict after week 8 (~2026-10-26), written by `tick`; then update `E-shadow` in `models/tuning/hypotheses.json` | waiting on the calendar |
...
| 9. Replay the period after the verdict: `python -m models.tuning replay --spec models/tuning/specs/replay_2026_w05_08.json`; then update `E-replay` in `models/tuning/hypotheses.json` | waiting on the calendar |
