...
| Doc | What it is |
| --- | --- |
| **[totals-modeling-guide.md](totals-modeling-guide.md)** | **Start here.** Living guide that merges the two reports below with the starter framework and the regression/priors conversation notes, then checks each claim against this repo's records: what to predict ($r^{\text{final}}$, $r^{\text{move}}$), which line eras can support ROI or CLV, the step from a predictive distribution to EV at the offered price, the model ladder, a validation calendar, and six open items. Adds no numbers of its own |
