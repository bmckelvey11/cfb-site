...
| Priority | Families |
| --- | --- |
| Priority experiment | GAMLSS/NGBoost; TabPFN walk-forward; simple era-weighted training around 2023; meta-labeling for bet selection. Dynamic latent-state ratings were tested and are a null: a Kalman random walk is 0.08 MAE worse than weekly ridge ([dynamic-ratings-2026-09-23.md](../../../docs/dynamic-ratings-2026-09-23.md)) |
...
| 7. Distribution and pricing | Built: integer predictive tables of the total (normal, empirical, joint home/away with overtime). The joint table is calibrated on 2021–25, with 80% intervals covering 80.8%, but its P(over) the open label has no skill: Brier 0.257 against a coin flip's 0.25 ([total-distributions-2026-09-23.md](../../../docs/total-distributions-2026-09-23.md)). The §8 pricing engine is built and verified on fixtures only (`models/tuning/market.py`) | Price real 2026 quotes in the Release E shadow period: the policy and the replay rules are frozen (`models/tuning/replay.py`, declared before week 5); the replay runs after the period verdict (~2026-10-26) |
...
| 10. Boosting, simulation, frontier | Frontier F1, dynamic Kalman ratings, is a null at its declared screen gate ([dynamic-ratings-2026-09-23.md](../../../docs/dynamic-ratings-2026-09-23.md)). Nothing else started | One family at a time, declared before tuning (tuning-lab plan §40 Release F) |
