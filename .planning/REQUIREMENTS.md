...
- [x] **FIX-02**: `aggregate_filter_value_rows` (backs `GET /filter-detail`) no longer double-counts a bet across multiple value buckets for total-system team/conference filters — per-value Record/ROI/Money in the modal table reconciles with the top-line backtest result.
