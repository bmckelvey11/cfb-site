...
- ✓ 4th bundled example system ("Neutral-Site & Indoor Unders") on the Example Systems tab — filters on `neutralSite`/`gameIndoors`/`venue_dome`, theory text cites the app's own independently-reproduced backtest (69-40-1, n=109, 63.30%, +20.66% ROI, p≈0.011) rather than the source doc's figures for a different 2-condition population, with honesty caveats (small sample, Bonferroni) intact; Copy to My Systems verified live — Phase 8 (DATA-01)
...
| A bundled example may filter on a weather-group feature only via a narrow, explicit, documented exception to the D-21 "no weather/season-to-date filters" test guard | `gameIndoors` (weather-group) is one of the 3 features the neutral-site/indoor-unders finding needs; the guard exists to prevent examples that guarantee zero live matches, but this example's near-zero-match weeks are already an accepted, documented consequence (08-CONTEXT.md) | ✓ Implemented — Phase 8, `WEATHER_FILTER_EXCEPTIONS = {("neutral-site-indoor-unders", "gameIndoors")}` keyed by exact `(example_name, feature_key)` pair, verified narrow (does not weaken the guard for any other example or key) |
...
---
*Last updated: 2026-08-27 after Phase 8 (Neutral-Site & Indoor Unders Example)*
