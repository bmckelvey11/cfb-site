...
|------|------|------|--------|
| 2026-08-28 | postseason-coverage-gap | `data/raw/games_*.json` and `lines_*.json` are `seasonType=regular` only for 1992-2024 (2025 is the exception, written by `upcoming.py` with `season_type=both`), so every bowl/CFP game is missing; 19 of 73 endpoints are season-type-scoped and inherit it, `PER_GAME` inherits it again via the `games_*` seed. 2025's 50 bowls sit in `games.csv` as **week 1** beside the regular openers. `stg.game` named source of record; a postseason pass would overwrite the regular rows, `--season-type both` is the fix. 469 of 515 lined FBS postseason games (2013-2025) missing from the backtest sample, 3.5%. Documented in `docs/data-coverage.md`; no code changed | complete |
