...
|------|------|------|--------|
| 2026-08-28 | garbage-time-endpoint-variants | Nine CFBD endpoints accept `excludeGarbageTime` and none was pulled with it; registered them as `_ngt` variants (`Endpoint.name` is the file prefix, so they land beside their twins and never overwrite) and pulled 2012-2025: 520 files, 543,288 rows, 0 failed. Not a narrowing filter — it drops the blowout plays feeding the aggregation, so it cannot be reproduced from dumps that store aggregates and not plays. Probed 2024 first: all nine real, and player-level endpoints *drop rows* (a player whose only snaps were garbage time has no qualifying plays left, player_usage 4,131 -> 3,696) while game/team-level keep every row and move values. Verified by margin, not row count: 9.5% of 0-7 pt games changed vs 99.5% of 28+ pt games, all 40 biggest blowouts differ, 228/262 one-score games byte-identical. Exposed and fixed `_scrape_season_week` silently dropping `endpoint.fixed`, which would have written unfiltered rows under the `_ngt` name with `resume` protecting them forever | complete |
| 2026-08-28 | postseason-backtest-impact | Pre-committed analysis plan then results for the 4 bundled example systems. Three barely move (<=0.15pp); `neutral-site-indoor-unders` drops 63.30% -> 55.98% (-7.32pp) because bowls ARE neutral-site games, so its sample doubles 109 -> 234. No postseason edge found on any system (BH q=0.912) but the nulls are uninformative by design: pre-run MDE 7.9-13.4pp vs a realistic 2-4pp edge, binding constraint is 13-16 clusters not bet count. Also fixed the bug this exposed: bowls sat at week 1 next to openers with no way to tell them apart | complete |
| 2026-08-28 | postseason-pass-for-week-mode-endpoints | Closed the seasonType gap for the last 6 endpoints (the `season_week` ones): `_scrape_season_week` now runs one pass per season type, postseason into `{name}_{season}_post_wk{week}.json`, so regular week 1 and postseason week 1 can no longer share a file. Postseason weeks are read off the games seed rather than assumed — 2020 has week 20, 2023 weeks 11-15, 2025 weeks 13-14, so week 1 alone would have dropped 32 games in 2025. Pulled 2012-2025: 100 files, 153,164 rows. Verified by row-set disjointness and a union match against live `both` (137+50=187). audit_coverage.py now expects the postseason files; its 5 absences per endpoint are upstream empties, probed and confirmed | complete |
| 2026-08-28 | close-postseason-seasontype-gap | `--season-type` defaulted to `regular` while CFBD's own default is `both`, so every dump was regular-season only; flipped the default on fetch/scrape and re-scraped the 13 SEASON-mode endpoints that accept it — `games.csv` 13,014 -> 13,515 (+501 bowl/playoff games, 586 priced postseason lines). The 6 SEASON_WEEK endpoints stay `regular`: postseason weeks restart at 1, so `both` merges regular wk1 with postseason wk1 into one `_wk1` file (game_team_stats 2024: 137 + 50 = 187). Verified regular-row counts unchanged per endpoint per season and `running_games_played` unmoved, so no bowl entered any prior-game window | complete |
