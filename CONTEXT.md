...
- **Quarter Kelly** — a quarter of the single-bet Kelly fraction (p·b − (1−p)) / b, then divided by 1 + (n−1)·ρ for n simultaneous bets at outcome correlation ρ. Full Kelly maximizes log growth under a tolerance the bankroll does not have; the fraction is the growth-vs-drawdown dial. (`kelly_unit` in `mc_combined_totals.py`)
...
- **Same-week shock** — one latent scoring-environment draw per week, ρ = 0.10 on the latent scale (outcome correlation ≈ 0.063), shared by every bet that Saturday through a Gaussian copula. Over-zero OVERs and Greenline UNDERs are pushed in opposite directions by it. The shock has one sign; a model-or-market failure that hurts both legs together is not modelled. (`research/bankroll/docs/mc-method-2026-09-17.md`)
