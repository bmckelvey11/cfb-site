...
def test_pairs_graphql_member_is_a_real_stg_gql_destination():
    ...
    # a test.
    stg_gql_names = set(GQL_ENTITY_TO_STG.values())
    for concept, gql_name, rest_name in PAIRS:
        assert gql_name in stg_gql_names, f"{concept}: {gql_name} not in GQL_ENTITY_TO_STG.values()"
