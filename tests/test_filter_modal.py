...
def test_feature_sidebar_js_search_collapse_contract():
    ...
    # Badge counts the same chips the rows render as solid pills, so the two
    # can never disagree.
    assert '[data-has-filter="1"]' in source
    # Collapse is a real button exposing its state.
    assert "aria-expanded" in source
    assert "is-collapsed" in source


def test_feature_sidebar_search_matches_the_stat_name(tmp_path):
    """A team-scoped stat renders its name in .stat-row__label while its buttons
    read only "Bet-side"/"Opponent". Matching button text alone would miss every
