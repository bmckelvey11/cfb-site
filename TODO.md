...
- [x] ~~`#pff-s7-trim-pull` **Finish trimming the PFF pull plan against 2025**~~ <!-- id: pff-s7-trim-pull --> — done 2026-09-10: all three cuts applied, then corrected the same day in `scripts/pull_pff_modeling.py` (13 team reports dropped — 12 leaderboard re-cuts plus `run-blocking`, which `pass-blocking` dominates on 19,206 cells, `team-rushing-direction` pulled once, `facet-passing-detail` pinned out); `audit_pff_pull.py` moved in lockstep. Planner delta measured at exactly −1,904 reads (3,997 → 2,093; 79 → 58 min a season, backfill 14.5 h → 10.6 h); `audit_pff_pull.py --season 2025` byte-identical to baseline, exit 0. Cost restated and the tier decision (13 dropped, 6 retained pending S6 gate 3) recorded in `docs/pff-ingest-plan.md`. Pinned by `tests/test_pff_audit.py`; 849 passed.
- [ ] `#pff-s6-backfill` **Backfill PFF 2014–2024 and finish 2026** <!-- id: pff-s6-backfill --> — S6, **still held, but the reason narrowed 2026-09-10.** Cost restated after `#pff-s7-trim-pull`: **10.6 h of metered calls, not 14.5** (58 min a season × 11). Gate 1 (S3+S5 land) ✅ and gate 2 (S7 lands) ✅; **gate 3 is now the only one open** — which of the six retained team reports a backfill needs. S7 deliberately left that to this gate because it is a modelling question, and gave it two facts: nothing in the warehouse reads a `team_report_*` file (all 19 target tables come from leaderboard exports), and the six cost 816 reads a season — the difference between 10.6 h and 9.0 h (the lever shrank from 3.7 h when two of the eight turned out to be duplicates). Also blocked on `#rotate-secrets-after-compromise` regardless: `PFF_API` is assumed disclosed, and eleven seasons of metered calls should not run on that. Done when `python scripts/audit_pff_pull.py` reports every backfilled season clean. P2 @needs-data
...
- [ ] `#oddsapi-warehouse-wiring` **Wire the-odds-api snapshots into the warehouse** <!-- id: oddsapi-warehouse-wiring --> — pulling on a schedule into `data/ingest/oddsapi/`, which `cfb_paths` deliberately does not glob; flatten + loader entries deferred by choice. **The stated join-key blocker is measured and smaller than recorded** (2026-09-10, [`docs/oddsapi-team-name-join-2026-09-10.md`](docs/oddsapi-team-name-join-2026-09-10.md), `python scripts/audit_oddsapi_team_names.py --list`): 170 of 173 snapshot names resolve to exactly one `core.dim_team` row under a mascot strip, **zero ambiguous**, 3 need an alias (App State, Southern Miss, Massachusetts); accents/apostrophes must be folded, not blanked. 39 FCS names all resolve, so the FCS worry does not hold. Still open before a flatten: the `(home, away, commence_time)` → `game_id` pairing under kickoff drift and neutral sites, and the nullable quota fields. Shape: flatten + pinned loader entries the way PFF's do (`cfb_system_maker/pff_schema.py`), failing loudly on an unresolved name rather than guessing. Done when the snapshots land in `stg` under pinned types. P2
