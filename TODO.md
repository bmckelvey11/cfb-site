...
- [x] ~~`#core-merge-bucket-c` **Merge the complementary pairs into `core`**~~ <!-- id: core-merge-bucket-c --> <!-- plan: docs/superpowers/plans/2026-09-08-warehouse-rationalization-master.md --> — done 2026-09-10: all 9 actionable pairs, in two passes. **Pass 1 (5 new tables):** full outer joins on the key, all guarded and key-unique — `core.dim_coach` 1,842 + `coach_name_conflicts` 2, `dim_draft_pick` 13,080, `dim_recruit` 93,363, `fact_team_talent` 2,430, `fact_coach_season` 12,564 + `coach_season_unmatched` 0. **Pass 2 (4 merges into existing tables)** — [`docs/core-merge-bucket-c-2026-09-10.md`](docs/core-merge-bucket-c-2026-09-10.md), `python scripts/audit_core_merges.py`: `conference` added `division` (256/256 match; `srName` fills 1/256, not carried); `game` repaired 3,714 home / 3,985 away conference ids that a non-injective name lookup had picked arbitrarily among 44 duplicate `dim_conference` names, and sent its 78,030 pre-2012 surplus to the new `core.fact_game_historical` per ADR-0001; `calendar` is a **measured no-op** (0 in-span gaps; taking its 2002–2011 rows would expand `fact_game` through `dim_week`'s bound). Every pre-existing `core` row count is unchanged, validated on a scratch copy first. Pinned by `tests/test_core_merges.py` (16). `lines` split out to `#core-fact-game-line-repoint` — it is not a merge. `recruiting_team` stays deferred (no id↔name bridge; see `#core-dim-team`).
- [ ] `#core-fact-game-line-repoint` **Repoint `core.fact_game_line` at `stg_gql.game_lines`** <!-- id: core-fact-game-line-repoint --> — split out of `#core-merge-bucket-c` 2026-09-10 because it is not a merge. `stg_gql.game_lines` is **already** merged — `line_source` reads cfbd 37,048 / actionnetwork 8,656 / cfbd+an 1,599 — while `_build_fact_game_line` unnests REST `stg.lines` and so builds the CFBD-only subset (39,006 rows, 12 providers). Five books have never reached `core`: circa 1,950, fanduel 1,949, betmgm 1,898, bet365 453, pinnacle 143. Grain gate already passes — `period='game'` is 47,303 rows, unique on `(gameId, linesProviderId)`. Shape: rewrite `_build_fact_game_line` to read `game_lines WHERE period='game'`, join `stg_gql.lines_provider` for `provider_key`, keep the existing column names and home-relative spread convention. **This is §7's full-game match-count gate** — measure REST offers matching a `period='game'` row on `(gameId, provider)` in both directions before building. `core.fact_game_line` feeds `eval_version_b.py` and `tests/test_spread_version_b.py`, so expect the slow suite to move; that is signal. Numbers: [`docs/core-merge-bucket-c-2026-09-10.md`](docs/core-merge-bucket-c-2026-09-10.md) §lines. Done when the 5 books appear in `core.fact_game_line`, the 12 shared providers' values are unchanged, and the slow-suite delta is explained line by line. P1
