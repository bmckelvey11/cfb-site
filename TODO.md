...
- [ ] `#pff-s6-backfill` **Backfill PFF 2014–2024 and finish 2026** <!-- id: pff-s6-backfill --> — S6. **All three gates are met as of 2026-09-10** — S3+S5 landed, S7 landed, and gate 3 (which team reports a backfill needs) is answered: **none**. The whole per-team report tier is dropped because nothing consumes it — no loader reads a `team_report_*` file and every target table in `docs/pff-warehouse-schema.md` comes from a leaderboard export. **Cost is now 9.0 h of metered calls, down from the 14.5 h originally recorded** (49 min a season × 11; 3,997 → 1,277 reads). Reversible: `TEAM_REPORTS` is an empty tuple, so restoring e.g. `pass-rush` for its `lhs_*`/`rhs_*` splits is one line plus 136 reads a season. **Blocked only on `#rotate-secrets-after-compromise`** — `PFF_API` is assumed disclosed and eleven seasons of metered calls should not run on it — and then a go/no-go on 9 hours. Done when `python scripts/audit_pff_pull.py` reports every backfilled season clean. P2 @needs-data
...
- [ ] `#oddsapi-warehouse-wiring` **Wire the-odds-api snapshots into the warehouse** <!-- id: oddsapi-warehouse-wiring --> — pulling on a schedule into `data/ingest/oddsapi/`, which `cfb_paths` deliberately does not glob. **Both halves of the stated join blocker are measured (2026-09-10) and neither is a mapping project.** Names: 170/173 resolve to one `core.dim_team` row, zero ambiguous, 3 aliases needed ([`docs/oddsapi-team-name-join-2026-09-10.md`](docs/oddsapi-team-name-join-2026-09-10.md)). Games: 98/98 events pair to exactly one `stg.games` row on the **unordered team pair**, zero ambiguous ([`docs/oddsapi-game-join-2026-09-10.md`](docs/oddsapi-game-join-2026-09-10.md), `python scripts/audit_oddsapi_game_join.py --list`). **Design: pair on the team pair within season, use kickoff only to split the 0.65% of FBS pair-seasons that are rematches** — kickoff cannot be a key, since Houston–Texas Tech shows the vendors differing by a full day and any sub-24h tolerance would drop it silently. Still open: the flatten shape and pinned types (mirror `cfb_system_maker/pff_schema.py`), the nullable quota fields, and re-running the join audit once TBD kickoffs (421 in 2026) and postseason games are actually in the snapshots — both untested, not clean. Done when the snapshots land in `stg` under pinned types. P2
