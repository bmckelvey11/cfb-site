...
- [ ] `#core-merge-bucket-c` **Merge the complementary pairs into `core`** <!-- id: core-merge-bucket-c --> <!-- plan: docs/superpowers/plans/2026-09-08-warehouse-rationalization-master.md --> — **partly done 2026-09-10: 5 of 9 actionable pairs.** Built as full outer joins on the key (one row per key, `_source` ∈ both/gql/rest), all guarded, all key-unique, applied to the live warehouse: `core.dim_coach` 1,842 + `coach_name_conflicts` 2, `dim_draft_pick` 13,080, `dim_recruit` 93,363, `fact_team_talent` 2,430, `fact_coach_season` 12,564 + `coach_season_unmatched` 0. Purely additive — every pre-existing `core` table has identical row counts. Pinned by `tests/test_core_merges.py`. **Still to do: the four pairs whose target already exists and is consumed** — `game` → `core.fact_game`, `lines` → `core.fact_game_line`, `conference` → `core.dim_conference`, `calendar` → `core.dim_week`. Those are rewrites of live tables the Flask app and backtests read, not additions, so they need their own change with consumer checks. **§7's full-game line match-count gate is therefore unexercised** — it lives on the lines merge. `recruiting_team` stays deferred (no id↔name bridge; see `#core-dim-team`). P1
